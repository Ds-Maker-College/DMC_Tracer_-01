/////////////////////////////////////////////////////
//    Sample soft for DMC_tracer simple course D2 
// 
...
void setup() {
 ...
 //----------------------------------
  ...
  while(1){
    ...
    display.print(F("Ver:Simple Course D2"));
...
void loop() {
  ...
  //センサが左から 白黒白白 と認識しているとき
  else if((line_sensor_l2 > thread_l2) && (line_sensor_l1 < thread_l1) && (line_sensor_r1 > thread_r1) && (line_sensor_r2 > thread_r2)){
    
    //左折小
    analogWrite(MT_PWM_L,vel_set_l - 25); 
    analogWrite(MT_PWM_R,vel_set_r);
    lineout_ready_L = 0;
    lineout_ready_R = 0;
    digitalWrite(LED,0);  //LED消灯
  ...
  //センサが左から 白白黒白 と認識しているとき
  else if((line_sensor_l2 > thread_l2) && (line_sensor_l1 > thread_l1) && (line_sensor_r1 < thread_r1) && (line_sensor_r2 > thread_r2)){

    //右折小
    analogWrite(MT_PWM_L,vel_set_l);
    analogWrite(MT_PWM_R,vel_set_r - 25);
    lineout_ready_L = 0;
    lineout_ready_R = 0;
    digitalWrite(LED,0);  //LED消灯
  }
