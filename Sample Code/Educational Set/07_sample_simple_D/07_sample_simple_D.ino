/////////////////////////////////////////////////////
//    Sample soft for DMC_tracer simple course D 
// 
...
void setup() {
 ...
 //----------------------------------
  ...
  while(1){
    ...
    display.print(F("Ver:Simple Course D"));
...
void loop() {
  ...
  long thread_l1 = 400; //左内側センサの白黒閾値
  long thread_r1 = 400; //左内側センサの白黒閾値
  ...
  //センサ値に応じた走行パターンを決定----------------------------------
  //左右のセンサが黒と認識しているとき
  if(line_sensor_l1 < thread_l1 && line_sensor_r1 < thread_r1){
  ...
  //左のセンサが黒　右のセンサが白と認識しているとき
  else if(line_sensor_l1 < thread_l1 && line_sensor_r1 > thread_r1){
